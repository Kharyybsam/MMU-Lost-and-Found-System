...
def reportfounditempage():
    if request.method == 'POST': 
        ...
        if len(itemname) < 1:
            ...
            new_founditems = Founditem(name=itemname,description=itemdescription, user_id=current_user.id,image_file=imagebase64,contact=itemcontact,location=location)  #providing the schema for the note 
...
def reportlostitempage():
    if request.method == 'POST': 
        ...
        if len(itemname) < 1:
            ...
            new_lostitems = Lostitem(name=itemname,description=itemdescription, user_id=current_user.id,image_file=imagebase64,contact=itemcontact,location=location)  #providing the schema for the note 
